...
describe('function interfaces', () => {
  it('set owner', () => {
    ...
    expect(func[declare]()).toBe('#define function setOwner(address) nonpayable returns ()');
  ...
  it('max', () => {
    ...
    expect(func.isDeclared).toBeFalse();
    expect(func[declare]()).toBe('#define function max(uint256, uint256) pure returns (uint256)');
    expect(func.isDeclared).toBeTrue();
