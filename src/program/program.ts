...
export class Program {
  ...
  /**
   * You can pass in `MAIN`, `CONSTRUCTOR`, `TEST` or any other `Macro` / `Fn` that
   * you have written. The idea is that if there is no `MAIN`, you can still compile the
   * code to be imported by other Huff codes.
   */
  ...
  constructor(entry: Compilable | Compilable[]) {
    ...
    if (this.entries.filter(e => e instanceof Main).length > 1) {
      throw new Error('there cant be multiple MAIN macros');
    }
    if (this.entries.filter(e => e instanceof Constructor).length > 1) {
      throw new Error('there cant be multiple CONSTRUCTOR macros');
    }
  }
  ...
  compile(
    fields: {
      /** Authors of this contract, usually "name <github>" */
      authors?: string[];
       ...
       * Each value in the array becomes a new line.
       */
      comments?: string[];
    ...
    const docs = [
      ...
      '/// @dev auto-generated by thuffscript',
    ];
...
`;
  ...
  /** Export compiled code to `path`. Will compile if not compiled already.  */
